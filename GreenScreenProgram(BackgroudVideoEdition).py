...
bgVideo="space.mp4"                     #Background_Video
cap=cv2.VideoCapture(ogVideo)                            
bg_vid=cv2.VideoCapture(bgVideo)                               
ret,frame=cap.read()
...
while 1:
    ...
    if ret:
        #BACKGROUND VIDEO OPERATIONS
        retbg,framebg=bg_vid.read()
        frame_counter+=1
        if frame_counter==int(bg_vid.get(cv2.CAP_PROP_FRAME_COUNT))-1:
            frame_counter=0
            bg_vid.set(cv2.CAP_PROP_POS_FRAMES,0)
        framebg=cv2.resize(framebg,(frame.shape[1],frame.shape[0])) #reshaping the background to fit the foreground
        ...
        #bitwise operations
        bg=cv2.bitwise_and(framebg,framebg,mask=mask)
